...
- **Timing (not AUTO):** set **Duration (seconds)** on the part and on timed graphic pieces (`headline` ILU, `l3d-headline`, `l3d-tema`, `l3d-mod`, …). Blueprints inherit part duration onto PGM L3D pieces when a piece has none, and derive part duration from the longest child graphic when the story has none — so a Mod story like “Gabriela Kajtárová” stays on air without manually duplicating DUR on every piece. Sofie **AUTO** only appears when blueprints set `autoNext` (VT / Intro / non-ILU GFX). ILU parts (with or without camera) do **not** autoNext — leave Start at `0` so they begin on Take.
