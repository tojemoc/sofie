...
- **Timing (not AUTO):** set **Duration (seconds)** on the story (part) and/or on timed graphic pieces. **Rundown Editor** inherits story duration onto PGM L3D pieces (`l3d-mod`, `l3d-headline`, `l3d-tema`, …) when a piece has none, and derives story duration from the longest child when the story has none — the same numbers are shown in the Dur column and exported to Sofie ingest. Blueprints pass ingest durations through unchanged. Sofie **AUTO** only appears when blueprints set `autoNext` (VT / Intro / non-ILU GFX). ILU parts (with or without camera) do **not** autoNext — leave Start at `0` so they begin on Take.
